...
export function AccountRegistrationManager({
  ...
  const addChildren = useMutation(api.registrations.addMyChildren);
  const updateChild = useMutation(api.registrations.updateMyChild);
  const activeChildren = useMemo(
    () => registeredChildren.filter((child) => child.status === "active"),
    [registeredChildren],
  );
  const availableSavedChildren = useMemo(
    () =>
      (account?.savedChildren ?? []).filter(
        (saved) =>
          !activeChildren.some(
            (child) =>
    ...
    [account?.savedChildren, activeChildren],
  );
  const [selected, setSelected] = useState<Id<"registrationChildren">[]>([]);
  const [editingId, setEditingId] = useState<Id<"registrationChildren"> | null>(null);
  ...
  function showError(error: unknown, fallback: string) {
    setStatus({
      type: "error",
      message: error instanceof Error ? cleanError(error.message) : fallback,
    });
  }
    ...
    <section className="panel manage-registration-children">
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                showError(error, "We could not claim these seats.");
              }
            }}
          >
      ...
      {showAdd && (
        <div className="participant-editor">
          ...
          {availableSavedChildren.length > 0 && (
            <div className="saved-child-picker">
              <span>Use a saved child</span>
              <div>
                {availableSavedChildren.map((child) => (
                  <button
                    type="button"
                    key={child.id}
                    onClick={() =>
          ...
          <label className="checkbox-row participant-waiver">
            ...
            />
            <span>
              I agree to The Forge participation terms, waiver, and photo release for this child.
            </span>
          </label>
          <button
            className="button button--red"
            ...
            onClick={async () => {
              ...
              try {
                await addChildren({
                  registrationId,
                  children: [normalizedDraft(addDraft)],
                  waiverAccepted,
                });
                ...
                showError(error, "We could not add this participant.");
              }
            }}
          >
      ...
      <div className="managed-child-list">
        {registeredChildren.map((child) => (
          <div
            className={`managed-child-card ${child.status === "cancelled" ? "is-cancelled" : ""}`}
            key={child.id}
          >
            <label className="managed-child">
              ...
              <span>
                ...
                <small>Age {child.age} · Born {formatBirthDate(child.birthDate)}</small>
              </span>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      showError(error, "We could not update this participant.");
                    }
                  }}
                >
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              showError(error, "We could not update the registration.");
            }
          }}
        >
  ...
  return (
    <div className="field-grid participant-fields">
      <label className="field">
        ...
        <input required value={draft.firstName} onChange={(e) => onChange({ ...draft, firstName: e.target.value })} />
      </label>
      <label className="field">
        <span>Last name</span>
        <input required value={draft.lastName} onChange={(e) => onChange({ ...draft, lastName: e.target.value })} />
      </label>
      <label className="field">
        <span>Birth date</span>
        <input required type="date" value={draft.birthDate} onChange={(e) => onChange({ ...draft, birthDate: e.target.value })} />
      </label>
      <label className="field">
        <span>Age</span>
        <input required type="number" min="1" max="21" value={draft.statedAge} onChange={(e) => onChange({ ...draft, statedAge: Number(e.target.value) })} />
      </label>
      <label className="field field--full">
        <span>Allergies</span>
        <input value={draft.allergies ?? ""} placeholder="None, or list allergies" onChange={(e) => onChange({ ...draft, allergies: e.target.value })} />
      </label>
      <label className="field field--full">
        <span>Notes</span>
        <textarea value={draft.notes ?? ""} placeholder="Anything the Forge team should know" onChange={(e) => onChange({ ...draft, notes: e.target.value })} />
      </label>
    </div>
  );
}
